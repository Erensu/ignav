...
static void primudata(vt_t *vt, const raw_t *raw)
{
    ...
    vt_printf(vt,"\n%3s %10s %6s %6s %6s %6s %6s %6s %6s\n",
              "No.","   timeTag","Accx","Accy","Accz","Gyrx","Gyry","Gyrz","Temp");
    for (i=0;i<pimu->n;i++) {
        vt_printf(vt,"%3d %10d %6.3lf %6.3lf %6.3lf %6.3lf %6.3lf %6.3lf %6.3lf\n",
                  i+1,pimu->data[i].imuc,
                  pimu->data[i].accl[0],pimu->data[i].accl[1],pimu->data[i].accl[2],
                  pimu->data[i].gyro[0],pimu->data[i].gyro[1],pimu->data[i].gyro[2],
                  pimu->data[i].temp);
    }
    vt_printf(vt,"\n");
}
    ...
    while (con->state) {
        ...
        switch (j) {
            ...
            case 19: /* shutdown */
                ...
                break;
            case 20: cmd_imudata(args,narg,con->vt);
                break;
