...
{
    int k; for (k=0;k<n;k++) o[k]=i[n-1-k];
}
...
extern long bin2dec(const unsigned char *b,int n)
{
    ...
    for (ret=0,s=0x01,i=n-1,j=0;i>=0;i--) {
        ret+=((b[i]&0x0F)   )*pow(16,j++); s=s<<4;
        ret+=((b[i]&0xF0)>>4)*pow(16,j++); s=s<<4;
    }
...
static int decode_navpvt(raw_t *raw)
{
    ...
    t[0]=U2(p+4); t[1]=U1(p+6); t[2]=U1(p+7);
    t[3]=U1(p+8); t[4]=U1(p+9); t[5]=U1(p+10);
    ...
    if      ((U1(p+21)&0x81)==0x81) raw->sol.stat=SOLQ_FIX;
    else if ((U1(p+21)&0x41)==0x41) raw->sol.stat=SOLQ_FLOAT;
    else if ((U1(p+21)&0x01)==0x01) raw->sol.stat=SOLQ_DGPS;
    else {
...
static int decode_esfraw(raw_t *raw)
{
    ...
    for (i=0;i<(raw->len-8)/8;i++) {
        ...
        switch (t[0]) {
            case 0x10: imus.accl[0]=bin2dec(t+1,3)*P2_10; f|=0x01; break;
            case 0x11: imus.accl[1]=bin2dec(t+1,3)*P2_10; f|=0x02; break;
            case 0x12: imus.accl[2]=bin2dec(t+1,3)*P2_10; f|=0x04; break;
            case 0x0E: imus.gyro[0]=bin2dec(t+1,3)*P2_12; f|=0x08; break;
            case 0x0D: imus.gyro[1]=bin2dec(t+1,3)*P2_12; f|=0x10; break;
            case 0x05: imus.gyro[2]=bin2dec(t+1,3)*P2_12; f|=0x20; break;
            case 0x0C: imus.temp   =bin2dec(t+1,3)*1E-02; f|=0x40; break;
        }
        imus.imuc=U4(p+i*8);

        /* add imu measurement data */
        if (f==0x7F) {
            addimudata(&raw->imut,&imus); f=0x00;
        }
    }
    return 4;
}
...
static int decode_ubxm8(raw_t *raw)
{
    ...
    /* decode ublox message */
    switch (type) {
        case ID_ESFINS: return decode_esfins(raw);
        case ID_ESFSTA: return decode_esfsta(raw);
        case ID_ESFRAW: return decode_esfraw(raw);
        case ID_NAVEOE: return decode_naveoe(raw);
    }
    if (raw->outtype) {
        sprintf(raw->msgtype,"UBX 0x%02X 0x%02X (%4d)",
                type>>8,type&0xF,raw->len);
    }
    return 0;
}
/* decode ublox raw message --------------------------------------------------*/
static int decode_ubxsol(raw_t *raw)
{
    ...
    switch (type) {
        ...
        case ID_NAVCLK : return decode_navclk (raw);
        case ID_NAVGPST: return decode_navgpst(raw);
