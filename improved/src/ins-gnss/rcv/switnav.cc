/*------------------------------------------------------------------------------
 * switnav.cc : Swift Navigation Binary Protocol decoder
*
...
static bandcode_t bandcode_map[CODE_COUNT] = {
        {0}
...
static int decode_msgobs(raw_t *raw)
{
    ...
    for (i = 0; i < num_obs && i < MAXOBS; i++, p += 17) {
        ...
        if ((CODE_INVALID != band_code) && (band_code < CODE_COUNT)) {
            code = bandcode_map[band_code].code;
            sys = bandcode_map[band_code].sys;
            freq = bandcode_map[band_code].freq;
        }
